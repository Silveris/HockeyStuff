...
// Get the data
d3.json("/api/v1.0/d", function(error, data) {
    data.forEach(function(d) {
        d.Season = +d.Season;
        d.Age = +d.Age;
        d.PIM/60min = +d.PIM/60min;
        d.Player = d.Player;
    });

    // Scale the range of the data
    x.domain([16, d3.max(data, function(d) { return d.Age; }) + 1]);
    y.domain([0, d3.max(data, function(d) { return d.PIM/60min; })]);
    ...
    svg.selectAll("dot")
      ...
      .filter(function(d) { return d.Season === 2011 })
        ...
        .attr("cy", function(d) { return y(d.PIM/60min); })

        .on("mouseover", function(d) {		
            div.transition()		
                .duration(200)		
                .style("opacity", .9);		
            div	.html(`${d.Player}<br>Age: ${d.Age}<br>PIM per 60 minutes: ${d.PIM/60min}`)	
    ...
    svg.append("text")
        ...
        .text("Penalty Minutes per 60 by Age");
